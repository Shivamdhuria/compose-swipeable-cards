...
object PageCurlShader {
     ...
     * - curlOrigin: Origin point of curl axis (normalized 0-1 coordinates)
     * - curlDirection: Direction vector of curl axis (normalized)
     * - curlDistance: Distance along axis from origin (normalized)
    ...
    private const val SHADER_SRC = """
        ...
        uniform vec2 curlOrigin;    // Origin point of curl axis (normalized)
        uniform vec2 curlDirection; // Direction vector of curl axis (normalized)
        uniform float curlDistance; // Distance along axis from origin
        ...
        vec4 main(vec2 fragCoord) {
            ...
            // Early exit: if no curl axis defined, show original content
            if (length(curlDirection) < 0.01) {
            ...
            // Use curl axis passed from Kotlin (already calculated)
            vec2 mouseDir = curlDirection;
            vec2 origin = curlOrigin;
            float mouseDist = curlDistance;
    ...
    /**
     * Helper to set curl axis parameters
     * @param axis CurlAxis containing origin, direction, and distance
     */
    internal fun RuntimeShader.setCurlAxis(axis: CurlAxis) {
        setFloatUniform("curlOrigin", axis.origin.x, axis.origin.y)
        setFloatUniform("curlDirection", axis.direction.x, axis.direction.y)
        setFloatUniform("curlDistance", axis.distance)
    }
}
